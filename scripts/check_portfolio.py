...
REQUIRED = (
    "Problem", "Context", "My Role", "Approach", "Architecture / Pipeline",
    "Key Technical Decisions", "Validation", "Results", "Limitations", "What I Learned",
    "Repository / Evidence",
)
PRIVATE_DIRS = {"private", "resumes", "applications", "exports", "raw", "cookies", "sessions"}
...
def forbidden_path(path: str) -> bool:
    ...
    if any(part.lower() in PRIVATE_DIRS for part in parts[:-1]):
        return True
    if path.startswith("data/") and path != "data/public/portfolio.json":
        return True
    return name.startswith((".env", "secrets.", "cookies.", "session.")) or bool(
        re.search(r"\.(?:db|sqlite\d*)(?:$|[-.])|\.(?:pem|key|pdf|docx)$", name)
    )
...
def tracked_guard(root: Path) -> None:
    ...
    for item in raw.split(b"\0"):
        ...
        path = root / relative
        if forbidden_path(relative) or path.is_symlink():
            raise ValueError("forbidden tracked path")
...
def check(root: Path, agent_root: Path | None = None, write: bool = False) -> PublicPortfolio:
    root = root.resolve()
    portfolio = load_portfolio(root / "data/public/portfolio.json")
    contract = (root / "scripts/public_contract.py").read_bytes()
    ...
    actual_schema = json.loads((root / "schemas/career-public-v1.schema.json").read_text(encoding="utf-8"))
    if actual_schema != expected_schema:
        raise ValueError("JSON Schema differs from executable contract")
    for project in portfolio.projects:
        narrative = (root / project.case_study).read_text(encoding="utf-8")
        marker = f"<!-- project: {project.project_id}; status: {project.status}; as_of: {portfolio.as_of} -->"
        if marker not in narrative or any(f"## {heading}\n" not in narrative for heading in REQUIRED):
    ...
    index = root / "generated/PROJECT_INDEX.md"
    ...
    for path in [root / "README.md", *sorted((root / "docs").glob("*.md")),
                 *sorted((root / "projects").glob("*.md")), index]:
